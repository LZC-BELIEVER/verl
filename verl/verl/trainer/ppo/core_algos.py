...
@register_policy_loss("acr")
def compute_policy_loss_acr(
    old_log_prob: torch.Tensor,
    log_prob: torch.Tensor,
    advantages: torch.Tensor,
    response_mask: torch.Tensor,
    ...
    """ACR: Adaptive Clipping Range (QuRL, ICLR 2026 under review).

    Addresses the training collapse issue of TIS (Truncated Importance Sampling) at long
    training horizons. When the behavior policy is truncated, TIS implicitly shrinks the
    clipping upper bound for positive-advantage tokens, causing over-clipping and collapse.

    ACR absorbs the proximal-to-behavior ratio r_it into the clipping term and keeps the
    upper bound fixed at (1+ε) for positive-advantage tokens:

        J_ACR = E_behav[ r_it * min(R*A, clip(R, 1-ε, (1+ε)/r_it) * A) ]

    where:
        R      = π_θ / π_proximal_old  (standard PPO ratio, computed from log_prob - old_log_prob)
        r_it   = min(π_proximal_old / π_behavior, C)  (proximal-to-behavior truncated ratio)
               = rollout_is_weights (pre-computed and detached in rollout_corr_helper)

    When r_it < 1 (behavior policy was truncated, i.e. π_prox/π_behav > C):
        clip_upper = (1+ε) / r_it > (1+ε)  →  more positive-advantage tokens can be updated.
    When r_it = 1 (no truncation): clip_upper = (1+ε)  →  identical to standard TIS.

    Args:
        old_log_prob: Log probabilities of the full-precision proximal (old) policy π_proximal_old,
            shape (batch_size, response_length). In the decoupled three-policy setup this is the
            full-precision old actor re-computed at training time.
        log_prob: Log probabilities of the current policy π_θ,
            shape (batch_size, response_length).
        advantages: Advantage estimates, shape (batch_size, response_length).
        response_mask: Valid token mask (1=valid, 0=padding),
            shape (batch_size, response_length).
        loss_agg_mode: Loss aggregation mode passed to `agg_loss`. Default: "token-mean".
        config: ActorConfig containing clip_ratio / clip_ratio_low / clip_ratio_high /
            clip_ratio_c and global_batch_info.
        rollout_is_weights: Pre-computed r_it = min(π_prox/π_behav, C), detached tensor,
            shape (batch_size, response_length). Produced by `compute_rollout_correction_weights`
            when rollout_is="acr". Falls back to ones (standard PPO) if None.

    Returns:
        Tuple of (pg_loss, pg_metrics):
            pg_loss: Scalar policy loss.
            pg_metrics: Dict with actor/pg_clipfrac, actor/ppo_kl, actor/pg_clipfrac_lower.

    Reference:
        QuRL: Efficient Reinforcement Learning with Quantized Rollout (ICLR 2026 under review),
    ...
    # r_it: proximal-to-behavior truncated ratio
    if rollout_is_weights is not None and rollout_log_prob is not None:
        # Recompute original ratio π_prox/π_behav from log probs
        log_ratio_prox_behav = old_log_prob - rollout_log_prob
        log_ratio_safe = torch.clamp(log_ratio_prox_behav, min=-20.0, max=20.0)
        ratio_prox_to_behav = torch.exp(log_ratio_safe)

        # Get threshold C from config
        rollout_is_threshold = 2.0  # default
        if hasattr(config, 'policy_loss') and config.policy_loss is not None:
            rollout_corr_config = config.policy_loss.get('rollout_correction', {})
            if rollout_corr_config is not None:
                rollout_is_threshold = rollout_corr_config.get('rollout_is_threshold', 2.0)

        # Compute r_it = min(1, C / (π_prox/π_behav))
        # When π_prox/π_behav > C, r_it < 1, enlarging the upper bound
        r_it = torch.min(
            torch.ones_like(ratio_prox_to_behav),
            rollout_is_threshold / ratio_prox_to_behav.clamp(min=1e-8)
        )
    else:
        r_it = torch.ones_like(ratio)  # fallback: equivalent to standard PPO

    # Adaptive upper clipping bound: (1+ε) / r_it
    # When r_it < 1 (behavior was truncated), upper bound expands beyond (1+ε),
    # allowing more positive-advantage tokens to receive gradient updates.
    dynamic_upper = (1 + clip_ratio_high) / r_it.clamp(min=1e-8)

    # Inner PPO clip with adaptive upper bound.
    # torch.clamp does not support mixed scalar/tensor bounds, so we implement:
    #   clip(ratio, 1-ε, dynamic_upper) = max(1-ε, min(ratio, dynamic_upper))
    ratio_clipped = torch.max(torch.full_like(ratio, 1 - clip_ratio_low), torch.min(ratio, dynamic_upper))
    pg_losses1 = -advantages * ratio
    pg_losses2 = -advantages * ratio_clipped
    clip_pg_losses1 = torch.maximum(pg_losses1, pg_losses2)
    pg_clipfrac = verl_F.masked_mean(torch.gt(pg_losses2, pg_losses1).float(), response_mask)

    # Dual-clip lower bound for negative advantages (standard dual-clip PPO)
    pg_losses3 = -advantages * clip_ratio_c
    clip_pg_losses2 = torch.min(pg_losses3, clip_pg_losses1)
    pg_clipfrac_lower = verl_F.masked_mean(
        torch.gt(clip_pg_losses1, pg_losses3) * (advantages < 0).float(), response_mask
    ...
    # Outer weight: multiply by r_it = min(π_prox/π_behav, C)
    pg_losses = pg_losses * r_it
