...
2. Model update staleness (training on trajectories from older checkpoints-speed-8b-ool)
...
def compute_rollout_correction_weights(
    ...
    # Validate threshold (can be float or string like "-_2.0" for IcePop)
    if isinstance(rollout_is_threshold, str):
        if "_" in rollout_is_threshold:
            try:
                parts = rollout_is_threshold.split("_")
                if parts[0] != "-":
                    lower = float(parts[0])
                    if lower <= 0:
                ...
                raise ValueError(f"Invalid rollout_is_threshold format: {rollout_is_threshold}. Expected '-_upper' (e.g., '-_2.0').") from e
        else:
            raise ValueError(f"Invalid rollout_is_threshold string format: {rollout_is_threshold}. Expected '-_upper' (e.g., '-_2.0').")
    elif rollout_is_threshold <= 0:
    ...
    # Support IcePop: if threshold is a string like "-_2.0", zero out weights > upper
    if isinstance(rollout_is_threshold, str) and "_" in rollout_is_threshold:
        # IcePop: hard trust region, zero out weights > upper (keep weights <= upper)
        parts = rollout_is_threshold.split("_")
        upper = float(parts[1])
        mask = rollout_is_weights <= upper
        rollout_is_weights = rollout_is_weights * mask.float()
    else:
        # TIS: clamp to upper threshold
...
def compute_is_metrics(
    ...
    # Parse threshold (can be float or string like "-_2.0")
    if isinstance(rollout_is_threshold, str) and "_" in rollout_is_threshold:
        parts = rollout_is_threshold.split("_")
        if parts[0] == "-":
            rollout_is_threshold_lower = 0.0  # No lower bound
            rollout_is_threshold_upper = float(parts[1])
        else:
            rollout_is_threshold_lower = float(parts[0])
            rollout_is_threshold_upper = float(parts[1])
    else:
        rollout_is_threshold_upper = float(rollout_is_threshold)
        rollout_is_threshold_lower = 1.0 / rollout_is_threshold_upper

    # Precompute log thresholds for accurate checks
    log_threshold_upper: torch.Tensor = torch.log(torch.tensor(rollout_is_threshold_upper, device=device))
    ...
    if rollout_is == "sequence":
        ...
        # Fraction of tokens exceeding thresholds
        rollout_is_above_threshold: torch.Tensor = rollout_is_weights > rollout_is_threshold_upper
    ...
    if mask_count > 1:
        weights_for_std: torch.Tensor = rollout_is_weights.clamp(
            min=rollout_is_threshold_lower, max=rollout_is_threshold_upper
        )
        mean_clamped: torch.Tensor = verl_F.masked_mean(weights_for_std, response_mask)
        rollout_is_var: torch.Tensor = (
            verl_F.masked_mean(weights_for_std.square(), response_mask) - mean_clamped.square()
    ...
    weights_for_ess: torch.Tensor = rollout_is_weights.clamp(min=rollout_is_threshold_lower, max=rollout_is_threshold_upper)
    mean_for_ess: torch.Tensor = verl_F.masked_mean(weights_for_ess, response_mask)
    is_weights_normalized: torch.Tensor = weights_for_ess / (mean_for_ess + 1e-8)  # Avoid division by zero
    ...
    if rollout_is_weights.dim() > 1:
        ...
        # Fraction of sequences with extreme weights
        metrics["rollout_is_seq_fraction_high"] = (seq_mean_weights > rollout_is_threshold_upper).float().mean().item()
...
def compute_offpolicy_metrics(
    ...
    - Model staleness (training on trajectories from older checkpoints-speed-8b-ool)
