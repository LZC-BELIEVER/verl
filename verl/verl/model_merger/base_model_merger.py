...
def parse_args():
    ...
    base_op_parser.add_argument("--local_dir", type=str, default=None, help="Path to the saved model checkpoints-speed-8b-ool.")
    base_op_parser.add_argument(
        "--tie-word-embedding",
        action="store_true",
    ...
    merge_parser = subparsers.add_parser("merge", parents=[base_op_parser], help="Merge model checkpoints-speed-8b-ool and save.")
...
class ModelMergerConfig:
    ...
    Args:
        ...
        is_value_model (bool): Whether the model is a value model (currently only Megatron
            supported). Defaults to False.
        local_dir (Optional[str]): Path to the saved model checkpoints-speed-8b-ool. Defaults to None.
...
class BaseModelMerger(ABC):
    """
    Abstract base class for merging distributed model checkpoints-speed-8b-ool into HuggingFace format.

    This class provides common functionality for converting model checkpoints-speed-8b-ool from different
    ...
    - merge: Convert and save checkpoints-speed-8b-ool to HuggingFace format
    - test: Validate merged checkpoints-speed-8b-ool against a reference model
