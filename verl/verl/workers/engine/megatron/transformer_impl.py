...
class MegatronEngine(BaseEngine):
    def __init__(
        ...
        if self._qat_enabled:
            # W8A8 uses plain TE layers with STE hooks, does not require non-vanilla bridge.
            # Only W4A16/W4A4 (ModelOpt path) need vanilla_mbridge=False.
            if self._qat_config.mode != "w8a8" and self.engine_config.vanilla_mbridge:
                raise ValueError(
                    "QAT (W4A16/W4A4) requires non-vanilla Megatron bridge. "
                    "Please set 'use_mbridge=True' and 'vanilla_mbridge=False'. "
                    "Note: W8A8 QAT is compatible with the default vanilla_mbridge=True."
                )
    ...
    def _build_tf_config(self):
        ...
        if use_mbridge:
            if self.vanilla_bridge:
                ...
                # W8A8 uses standard TE layers with STE hooks, skip ModelOpt patching
                if self._qat_enabled and self._qat_config.mode != "w8a8":
    ...
    def initialize(self):
        ...
        if self._qat_enabled and not self.engine_config.forward_only:
            if self._qat_config.mode == "w8a8":
                from verl.utils.fp8_qat import apply_fp8_w8a8_qat_hooks

                self._fp8_qat_hook_handles = apply_fp8_w8a8_qat_hooks(self.module, self._qat_config)
            else:
    ...
    def save_checkpoint(
        ...
        **kwargs,
        ...
        Args:
            ...
            max_ckpt_to_keep: Maximum number of recent checkpoints-speed-8b-ool to retain.
        """
    ...
    def get_per_tensor_param(self):
        ...
        # QAT Guard 3: Export quantized weights for w4a16/w4a4 modes
        if self._qat_enabled and self._qat_config.mode in ["w4a16", "w4a4"]:
            from verl.utils.fp8_qat import export_qat_weights
            logger.info(f"Exporting QAT weights for mode={self._qat_config.mode}")
            per_tensor_param = export_qat_weights(
                per_tensor_param,
                mode=self._qat_config.mode,
                group_size=self._qat_config.group_size,
                ignore_patterns=self._qat_config.ignore_patterns,
            )
