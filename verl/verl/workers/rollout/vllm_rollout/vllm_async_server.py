...
class ExternalZeroMQDistributedExecutor(Executor):
    ...
    def _init_executor(self) -> None:
        ...
        for address in addresses:
            ...
            self._socket_locks.append(threading.Lock())
    ...
    def collective_rpc(
        ...
        **kwargs_extra: Any,
        ...
        if non_block:
            # Send and recv in the same thread per socket, holding the per-socket
            # lock for the full round-trip, so concurrent non_block calls cannot
            # interleave sends on the same REQ socket.
            futures = []
            for socket, lock in zip(self.sockets, self._socket_locks):
                future = Future()

                def _send_recv(sock, lk, fut, msg):
                    with lk:
                        try:
                            sock.send(msg)
                ...
                thread = threading.Thread(target=_send_recv, args=(socket, lock, future, message))
                thread.daemon = True
                thread.start()
            ...
            # Blocking: acquire all locks, fan-out sends so TP workers start
            # collective ops simultaneously, then fan-in recvs.
            for lock in self._socket_locks:
                lock.acquire()
