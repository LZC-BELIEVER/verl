...
EXP_NAME="fp8-calibrated-kvcache-verl-${TIMESTAMP}"
...
# Use pre-calibrated FP8 model (contains k_scale/v_scale baked in).
# calculate_kv_scales is NOT set (defaults to False) so vLLM loads
# static scales from the checkpoint rather than computing them on-the-fly.
PYTHONUNBUFFERED=1 python3 -m verl.trainer.main_ppo \
 ...
 actor_rollout_ref.model.path=/lanzichang/models/Qwen2.5-Math-7B-FP8-KV-calibrated \
 actor_rollout_ref.actor.optim.lr=1e-6 \
 actor_rollout_ref.actor.ppo_mini_batch_size=128 \
 actor_rollout_ref.actor.use_dynamic_bsz=${use_dynamic_bsz} \
 actor_rollout_ref.actor.ppo_max_token_len_per_gpu=$(( 20 * 1024 )) \
 ...
 trainer.project_name='FP8-RL-7B-calibrated' \
