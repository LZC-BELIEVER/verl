#!/usr/bin/env bash
# ============================================================
# Worst-5 Benchmark: Pure Baseline (原模型测试)
# 运行5次，每次直接测试原模型在测试集上的准确率
# ============================================================

TIMESTAMP=$(date +%Y-%m-%d-%H-%M-%S)
BASE_EXP_NAME="worst5-pure-baseline-8b-${TIMESTAMP}"
...
echo "Worst-5 Benchmark: Pure Baseline (Original Model)" > $SUMMARY_FILE
...
for RUN_ID in 1 2 3 4 5; do
    ...
    python3 -m verl.trainer.main_ppo \
        ...
        trainer.total_epochs=1 2>&1 | tee $RUN_OUTPUT_DIR/eval.log

    EVAL_EXIT_CODE=$?
    END_TIME=$(date +%s)
    ELAPSED=$((END_TIME - START_TIME))

    echo "Evaluation finished after ${ELAPSED}s with exit code: $EVAL_EXIT_CODE"
    echo "Run ${RUN_ID}: Evaluation time ${ELAPSED}s, exit code ${EVAL_EXIT_CODE}" >> $SUMMARY_FILE
    ...
    # 从validation目录中提取验证准确率
    echo "Extracting validation accuracy..."
...
" 2>/dev/null)
        echo "Validation accuracy (step ${LAST_VAL_STEP}): $LAST_VAL_ACC"
    else
    ...
    echo "Run ${RUN_ID}: Validation accuracy = ${LAST_VAL_ACC}" >> $SUMMARY_FILE
