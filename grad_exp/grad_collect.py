...
class GradCollectTrainer(RayDAPOTrainer):
    ...
    def fit(self):
        ...
        # Welford accumulator: keeps running mean / M2 on disk, so each
        # per-batch grad file can be discarded right after it's folded in.
        from welford import WelfordAccumulator

        accumulator = WelfordAccumulator(state_dir=grad_save_dir)
        if accumulator.n > 0:
            print(
                f"[grad_collect] resumed welford state with n={accumulator.n}",
                flush=True,
            )
        ...
        for _epoch in range(self.config.trainer.total_epochs):
            for batch_dict in self.train_dataloader:
                ...
                # ---- fold into Welford accumulator and drop the file ----
                grad_path = os.path.join(grad_save_dir, f"batch_{collected:04d}.pt")
                sample = torch.load(grad_path, map_location="cpu")
        ...
        pbar.close()
        accumulator.finalize(out_dir=grad_save_dir, sample_variance=True)
        # remove intermediate welford checkpoint files; keep only final stats
        for fname in ("welford_mean.pt", "welford_M2.pt", "welford_meta.json"):
            fpath = os.path.join(grad_save_dir, fname)
            if os.path.exists(fpath):
                os.remove(fpath)
        print(
            f"[grad_collect] folded {collected} batches via Welford -> "
            f"{grad_save_dir}/grad_mean.pt, grad_var.pt"
        )
