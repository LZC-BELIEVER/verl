#!/usr/bin/env bash
# ============================================================
# Gradient collection (FP8 vLLM rollout + Vanilla): save raw gradient from first batch
# ============================================================
ray stop --force || true
pkill -9 ray || true
pkill -9 python || true
...
EXP_NAME="grad_collect-fp8_vanilla-raw-${TIMESTAMP}"
OUTPUT_DIR="/lanzichang1/new_verl/checkpoints/$EXP_NAME"
GRAD_DIR="/lanzichang1/new_verl/grads/ori_grad/fp8_vanilla"
mkdir -p $OUTPUT_DIR
mkdir -p $GRAD_DIR
...
export PYTHONPATH="/lanzichang1/new_verl/verl:/lanzichang1/new_verl/scripts/grad_exp/ori_grad:${PYTHONPATH}"
...
# ---- Rollout Correction (Vanilla: no truncation) ----
rollout_is=token
rollout_is_threshold=1e9
rollout_rs=null
...
# Grad collection knobs - ONLY 1 BATCH
NUM_GRAD_BATCHES=1
BATCH_SIZE=32

PYTHONUNBUFFERED=1 python3 /lanzichang1/new_verl/scripts/grad_exp/ori_grad/grad_collect_raw.py \
    ...
    trainer.project_name='grad_collect_raw' \
