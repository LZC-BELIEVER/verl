#!/usr/bin/env bash
# ============================================================
# Gradient collection (FP8 vLLM rollout, vanilla / no TIS) on the MATH dataset:
# no optimizer step. Samples 200 batches of 32 prompts each from
# /lanzichang1/math/train.parquet, folds per-batch grads via Welford into
#   /lanzichang1/new_verl/grads/math/fp8_vanilla/{grad_mean.pt,grad_var.pt}
# ============================================================
ray stop --force || true
pkill -9 ray || true
pkill -9 python || true
...
EXP_NAME="grad_collect-math-fp8_vanilla-${TIMESTAMP}"
OUTPUT_DIR="/lanzichang1/new_verl/checkpoints/$EXP_NAME"
GRAD_DIR="/lanzichang1/new_verl/grads/math/fp8_vanilla"
mkdir -p $OUTPUT_DIR
mkdir -p $GRAD_DIR
...
# ---- Rollout Correction (vanilla: IS effectively disabled via huge threshold) ----
rollout_is=token
...
PYTHONUNBUFFERED=1 python3 /lanzichang1/new_verl/scripts/grad_exp/grad_collect.py \
    ...
    data.train_files=/lanzichang1/math/train.parquet \
