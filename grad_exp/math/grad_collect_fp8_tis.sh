#!/usr/bin/env bash
# ============================================================
# Gradient collection (FP8 vLLM rollout + TIS) on the MATH dataset: no optimizer step.
# Samples 200 batches of 32 prompts each from /lanzichang1/math/train.parquet,
# folds per-batch grads via Welford into
#   /lanzichang1/new_verl/grads/math/fp8/{grad_mean.pt,grad_var.pt}
# ============================================================
ray stop --force || true
pkill -9 ray || true
pkill -9 python || true
...
EXP_NAME="grad_collect-math-fp8_tis-${TIMESTAMP}"
OUTPUT_DIR="/lanzichang1/new_verl/checkpoints/$EXP_NAME"
GRAD_DIR="/lanzichang1/new_verl/grads/math/fp8_tis"
mkdir -p $OUTPUT_DIR
mkdir -p $GRAD_DIR
